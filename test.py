...
from utils.plots import plot_images, plot_images_0, output_to_target, plot_study_txt
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(out):
            ...
            if nl:
                ...
                tcls_tensor = labels_p[:, 0]

                # target boxes
                tbox = xywh2xyxy(labels_p[:, 1:5])
                scale_coords(img[si].shape[1:], tbox, shapes[si][0], shapes[si][1])  # native-space labels
                if plots:
                    confusion_matrix.process_batch(predn, torch.cat((labels_p[:, 0:1], tbox), 1))
        ...
        if plots and batch_i < 3:
            ...
            Thread(target=plot_images_0, args=(img, output_to_target(out), paths, f, names), daemon=True).start()
