...
def create_dataloader(path, imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
                      rank=-1, world_size=1, workers=8, image_weights=False, quad=False, prefix='', nc = 228):
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
                 cache_images=False, single_cls=False, stride=32, pad=0.0, prefix='', nc = 228):
        self.img_size = img_size
    ...
    def __getitem__(self, index):
        ...
        if self.augment:
            ...
            if random.random() < hyp['paste_in']:
                sample_labels, sample_images, sample_masks, sample_afters = [], [], [], [] 
                while len(sample_labels) < 30:
                    sample_labels_, sample_images_, sample_masks_, sample_after_ = load_samples(self, random.randint(0, len(self.labels) - 1))
                    sample_labels += sample_labels_
                    sample_images += sample_images_
                    sample_masks += sample_masks_
                    sample_after += sample_after_
                    #print(len(sample_labels))
                    if len(sample_labels) == 0:
                        break
                labels = pastein(img, labels, sample_labels, sample_images, sample_masks, sample_afters)
        ...
        labels_out = torch.zeros((nL, 6 + self.nc))
        if nL:
...
def load_samples(self, index):
    ...
    # Augment
    #img4, labels4, segments4 = remove_background(img4, labels4, segments4)
    sample_labels, sample_images, sample_masks, sample_after = sample_segments(img4, labels4, segments4, probability=0.5)

    return sample_labels, sample_images, sample_masks, sample_after
    ...
    if probability and n:
        ...
        for j in random.sample(range(n), k=round(probability * n)):
            ...
            if (ioa < 0.30).all():  # allow 30% obscuration of existing labels
                labels = np.concatenate((labels, [[l[0], *box] + l[5:]]), 0)
...
def pastein(image, labels, sample_labels, sample_images, sample_masks, sample_afters):
    # Applies image cutout augmentation https://arxiv.org/abs/1708.04552
    h, w = image.shape[:2]
        ...
        if (ioa < 0.30).all() and len(sample_labels) and (xmax > xmin+20) and (ymax > ymin+20):  # allow 30% obscuration of existing labels
            ...
            if (r_w > 10) and (r_h > 10):
                ...
                if m_ind.astype(np.int32).sum() > 60:
                    ...
                    if len(labels):
                        labels = np.concatenate((labels, [[sample_labels[sel_ind], *box] + sample_afters[sel_ind]]), 0)
                    else:
                        labels = np.array([[sample_labels[sel_ind], *box] + sample_afters[sel_ind]])
