...
class ComputeLoss:
    ...
    def build_targets(self, p, targets):
        ...
        gain = torch.ones(7 + targets.shape[1] - 6, device=targets.device).long()  # normalized to gridspace gain
...
class ComputeLossAuxOTA:
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model   
        ...
        for i in range(self.nl):  # layer index, layer predictions
            ...
            if n:
                ...
                # Classification
                selected_tcls = torch.where(targets[i][:, 6:] == 1)[1].long()
                if self.nc > 1:  # cls loss (only if multiple classes)
                    t = torch.full_like(ps[:, 5:], self.cn, device=device)  # targets
                    t[torch.where(targets[i][:, 6:] == 1)[0], selected_tcls] = self.cp
                    lcls += self.BCEcls(ps[:, 5:], t)  # BCE
            ...
            if n_aux:
                ...
                # Classification
                selected_tcls_aux = torch.where(targets_aux[i][:, 6:] == 1)[1].long()
                if self.nc > 1:  # cls loss (only if multiple classes)
                    t_aux = torch.full_like(ps_aux[:, 5:], self.cn, device=device)  # targets
                    t_aux[torch.where(targets_aux[i][:, 6:] == 1)[0], selected_tcls_aux] = self.cp
    ...
    def find_5_positive(self, p, targets):
        ...
        gain = torch.ones(7, device=targets.device).long()  # normalized to gridspace gain
        ai = torch.arange(na, device=targets.device).float().view(na, 1).repeat(1, nt)  # same as .repeat_interleave(nt)
        targets = torch.cat((targets[:,:6].repeat(na, 1, 1), ai[:, :, None]), 2)  # append anchor indices
