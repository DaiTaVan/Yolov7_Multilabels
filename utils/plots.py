...
def output_to_target_v2(output):
    # Convert model output to target format [batch_id, class_id, x, y, w, h, conf]
    targets = []
    for i, o in enumerate(output):
        for *box, conf, cls in o.cpu().numpy():
...
def plot_images_0(images, targets, paths=None, fname='images.jpg', names=None, max_size=640, max_subplots=16):
...
def plot_images(images, targets, paths=None, fname='images.jpg', names=None, max_size=640, max_subplots=16, nc = 228):
    # Plot image grid with labels
    ...
    for i, img in enumerate(images):
        ...
        if len(targets) > 0:
            ...
            for j, box in enumerate(boxes.T):
                # cls = int(classes[j])
                cls_list = torch.where(classes_lists[j] == 1)[0]
                # color = colors[cls % len(colors)]
